...
public class BaseEnterOutProcess extends SendRequest {
    ...
    String[][] constructParams(TLVMessage request) throws Exception {
        if (sq){
            ...
            jsonObj.put("divisionNo",sqDivisionNo);
            jsonObj.put("sqGateNo",sqGateNo);
            jsonObj.put("vehicleNo",request.getValue());
            jsonObj.put("accessType", accessType());
            jsonObj.put("scanTime",transferDate((String) request.getNextValue(0)));
            json = root.toString();
            return new String[0][];
        }
        return null;
