...
@Service
public class YinLianCarIn extends SendRequest {
    @Value("${out_address}")
    String outAddress;
    ...
    TLVMessage constructMessage(TLVMessage ret, TLVMessage request, JSONObject retJson, IoSession session) {
    	...
    	System.out.println("++++++++++银联，入场，获得TLV数据++++++++++");
...
//    	int version = request.getNextInt(_count++);//版本号，填1 
    	...
    	//第二步：调用银联
    	JSONObject rsp = YL.noticeEntrance(syncId, plateNumber, startTime);
    	
    	System.out.println("<<<<<<<<<<<<<<<银联，入场，返回JSON数据>>>>>>>>>>>>>>>>>>>");
...
//		停车记录唯一标识(订单号) string;
        ...
        System.out.println("[[[[[[[[[[[[[[[银联，入场，返回TLV数据]]]]]]]]]]]]]]");
